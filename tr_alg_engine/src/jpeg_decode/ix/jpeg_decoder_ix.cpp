#include "jpeg_decode/ix/jpeg_decoder_ix.h"
#include <mutex>
template <typename T>
...
bool CudaJpegDecode::Decode(const uchar *image, const int length, GpuMat& dst, bool pipelined)
{
    checkCudaErrors(cudaSetDevice(device->get_device_id()));
    cudaStream_t stream;
    ...
    GpuMat c1(heights[0], widths[0], CV_8UC1,1), c2(heights[0], widths[0], CV_8UC1,1), c3(heights[0], widths[0], CV_8UC1,1);
...
//        checkCudaErrors(nvjpegStateAttachDeviceBuffer(nvjpeg_decoupled_state_, device_buffer_));
        int buffer_index = 0;
        throw std::runtime_error("ix not support pipeline mode");
...
//        checkCudaErrors(nvjpegDecodeJpegTransferToDevice(nvjpeg_handle_, nvjpeg_decoder_, nvjpeg_decoupled_state_, jpeg_streams_, stream));
  //      checkCudaErrors(nvjpegDecodeJpegDevice(nvjpeg_handle_, nvjpeg_decoder_, nvjpeg_decoupled_state_, &iout, stream));
    ...
    dst.re_init(heights[0], widths[0], CV_8UC3,3);
    dst.from_channels(c1,c2,c3);
...
Jpeg_Decoder_ix::Jpeg_Decoder_ix(int max_wait, int max_decoder) : Jpeg_Decoder(max_wait, max_decoder) {
    format=nvjpegOutputFormat_t::NVJPEG_OUTPUT_BGR;

};

Jpeg_Decoder_ix::~Jpeg_Decoder_ix() {

};
void Jpeg_Decoder_ix::init(std::vector<std::shared_ptr<Device_Handle>> devices_handles, int init_cnt)
{
...
std::shared_ptr<QyImage> Jpeg_Decoder_ix::decode(const std::vector<unsigned char> &data)
{
    ...
    if (select_idx >= 0)
    {
        ...
        result = from_data(dst.data, dst.element_size, dst.step, dst.cols, dst.rows, decoders[select_idx].device, false);
...
int Jpeg_Decoder_ix::add_instance(std::shared_ptr<Device_Handle> device)
{
    std::unique_lock lock(mutex);
    if (device == nullptr)
...
int Jpeg_Decoder_ix::get_instance_idx()
{
    std::unique_lock lock(mutex);
...
extern "C" std::shared_ptr<Jpeg_Decoder> get_jpeg_decoder(int max_wait,int max_decoder){
    std::shared_ptr<Jpeg_Decoder_ix> decoder=std::make_shared<Jpeg_Decoder_ix>(max_wait,max_decoder);
