#include "network_engine/torch/network_kernel_torch.h"
...
Network_kernel_torch::Network_kernel_torch(std::shared_ptr<Device_Handle> handle, std::string file_path, std::string model_name, std::vector<Shape_t> &input_shapes, int max_instance) : Network_kernel(handle, file_path, model_name, input_shapes, max_instance)
{
    ...
    {
        this->net_instances.push_back(torch::jit::load(file_path, t_handle->handle));
#ifdef USE_FP16
...
int Network_kernel_torch::forward(std::vector<std::shared_ptr<QyImage>> &inputs, std::vector<Output> &outputs)
{
    ...
    if (have_data == false)
    {
        ...
        switch(check_need_scale_offset(i)){
            ...
            {
                throw Alg_Module_Exception("Error:\t input data type unknown cv mat depth " + std::to_string(type), this->model_name, Alg_Module_Exception::Stage::inference); // 鑾峰彇妯″瀷鎺ㄧ悊瀹炰緥寮傚父锛屼竴鑸槸鍥犱负妯″瀷瀹炰緥杩樻湭鍒涘缓
            }
    ...
    for (int i = 0; i < inputs_t.size(); i++)
    {
        ...
        if (input_shapes[i].num_dims != current_shape.size())
        {
            ...
            {
                throw Alg_Module_Exception("Error:\t input data shape mismatch", this->model_name, Alg_Module_Exception::Stage::inference); // 鑾峰彇妯″瀷鎺ㄧ悊瀹炰緥寮傚父锛屼竴鑸槸鍥犱负妯″瀷瀹炰緥杩樻湭鍒涘缓
                return -1;
            }
        }
    ...
    torch::jit::IValue instance_outputs;
    instance_outputs = net_instance.forward(instance_inputs);
...
std::vector<Shape_t> Network_kernel_torch::get_input_shapes()
{
    ...
    throw Alg_Module_Exception("Error:\t input shape not set in libtorch mode", this->model_name, Alg_Module_Exception::Stage::load_model); // 鑾峰彇妯″瀷鎺ㄧ悊瀹炰緥寮傚父锛屼竴鑸槸鍥犱负妯″瀷瀹炰緥杩樻湭鍒涘缓
    return cache_inputs_shapes;
};
