...
public:
    ...
    virtual int from_string_(std::string cfg_str) {
        ...
        if (this->cfg_float.find("min_car_width")==this->cfg_float.end()) {
            this->cfg_float["min_car_width"] = 400;
        }
        if (this->cfg_float.find("min_car_height")==this->cfg_float.end()) {
            this->cfg_float["min_car_height"] = 300;
        }
...
//    virtual bool detect_license_plate(string channel_name, bm_handle_t &handle, bm_image &input_image, std::vector<Result_Detect> &vehicles, std::vector<Result_item_License_Plate_Detection> &licenses);
    virtual bool detect_license_plate(std::string channel_name, std::map<std::string,std::shared_ptr<InputOutput>> &input, std::vector<Result_Detect> &vehicles, std::vector<Result_item_License_Plate_Detection> &licenses);
